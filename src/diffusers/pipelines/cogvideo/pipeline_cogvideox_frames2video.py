...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> pipe = CogVideoXFramesToVideoPipeline.from_pretrained("THUDM/CogVideoX-5b-I2V", torch_dtype=torch.bfloat16)
        >>> pipe.to("cuda")
...
class CogVideoXFramesToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def prepare_latents(
        ...
        if not self.vae.config.invert_scale_latents:
            ...
            # This is awkward but required because the CogVideoX team forgot to multiply the
            # scaling factor during training :)
            frames_latents = 1 / self.vae_scaling_factor_image * frames_latents
        ...
        if self.transformer.config.patch_size_t is not None:
            first_frame = frames_latents[:, :frames_latents.size(1) % self.transformer.config.patch_size_t, ...]
            frames_latents = torch.cat([first_frame, frames_latents], dim=1)
